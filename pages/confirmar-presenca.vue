<template>
  <section class="mx-auto max-w-3xl space-y-6">
    ...
    <div v-else-if="success" class="card p-8 text-center">
      ...
      <div v-if="meetingInfo" class="mt-6 w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left dark:border-dark-border dark:bg-dark-card">
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-[#a1a1aa]">
          {{ formatDate(meetingInfo.date) }} • {{ meetingInfo.start_time?.slice(0, 5) }} – {{ meetingInfo.end_time?.slice(0, 5) }}
        </p>
      </div>
    ...
    <div v-else class="card p-8">
      ...
      <h2 class="mt-2 text-3xl font-extrabold text-[#1E293B] dark:text-[#fafafa]">Sua presença na reunião</h2>
      <p class="mt-3 text-sm text-[#94A3B8] dark:text-[#a1a1aa]">
        Se você não possui conta, apenas informe seu nome para confirmar presença.
      </p>
      <form class="mt-6 grid gap-5 sm:grid-cols-2" @submit.prevent="confirmPresence">
        <div v-if="!isLoggedIn" class="sm:col-span-2">
          <label class="text-xs font-bold uppercase tracking-widest text-[#94A3B8] dark:text-[#a1a1aa]">Nome completo</label>
          <input v-model="guestName" class="input-soft mt-2" placeholder="Digite seu nome" :required="!isLoggedIn" />
        </div>
        <div v-if="!isLoggedIn" class="sm:col-span-2">
...
const loadMeetingInfo = async () => {
  if (meetingToken.value) {
    const { data } = await supabase
      ...
      .single()
    if (data) meetingInfo.value = data as Meeting
  } else if (token.value) {
    const { data } = await supabase
      .from('meeting_participants')
      .select('meeting_id, meetings:meeting_id(*)')
      .eq('invite_token', token.value)
      .single()
    if (data?.meetings) meetingInfo.value = data.meetings as unknown as Meeting
  }
}

const confirmPresence = async () => {
  if (!token.value && !meetingToken.value) {
    errorMsg.value = 'Token de convite não encontrado na URL.'
    return
  }
